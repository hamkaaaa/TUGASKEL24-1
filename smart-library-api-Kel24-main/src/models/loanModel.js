...
export const LoanModel = {
  ...
  try {
    ...
    // 1. Update status ke RETURNED dan isi return_date
    const res = await client.query(
      "UPDATE loans SET status = 'RETURNED', return_date = CURRENT_DATE WHERE id = $1 AND status = 'BORROWED' RETURNING book_id",
      [loan_id]
    );
    if (res.rows.length === 0) throw new Error('Transaksi tidak ditemukan atau sudah dikembalikan.');

    // 2. Tambah stok available_copies secara otomatis
    await client.query('UPDATE books SET available_copies = available_copies + 1 WHERE id = $1', [res.rows[0].book_id]);
